...
# def process_file(noisy_file, args, model):
#     filename = noisy_file.split('/')[-1]
...
#     # Reverse sampling
#     sampler = model.get_pc_sampler(
#         'reverse_diffusion', args.corrector, Y.cuda(), N=N,
#         corrector_steps=corrector_steps, snr=snr)
#     sample, _ = sampler()
...
#     # 使用tqdm创建进度条
#     with tqdm(total=len(noisy_files), desc="Enhancement") as pbar:
#         # 使用多进程并行处理文件
#         with ProcessPoolExecutor(max_workers=4) as executor:  # 这里可以调整max_workers来控制并发度
#             futures = [executor.submit(process_file, noisy_file, args, model) for noisy_file in noisy_files]
